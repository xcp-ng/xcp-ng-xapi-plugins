...
class TestLvmCommands:
    ...
    def test_list_logical_volumes_with_filter(self, run_command):
        ...
        run_command.assert_called_with([
            ...
            '-olv_name,vg_name,lv_attr,lv_size,pool_lv', TESTED_VG_NAME
        ])
    ...
    def test_create_physical_volume(self, run_command):
        ...
        assert create_physical_volume(None, {
            'devices': TESTED_DEVICES
        }) == '{}'
        run_command.assert_called_with(['pvcreate'] + TESTED_DEVICE_ARRAY + ['-qq'], check=False)
    ...
    def test_create_physical_volume_with_force(self, run_command):
        ...
        run_command.assert_called_with(['pvcreate'] + TESTED_DEVICE_ARRAY + ['-ff', '-y'], check=False)
    ...
    def test_create_physical_volume_and_ignore_fs(self, run_command):
        ...
        assert create_physical_volume(None, {
            'devices': TESTED_DEVICES,
            'ignore_existing_filesystems': '1'
        }) == '{}'
        run_command.assert_called_with(['pvcreate'] + TESTED_DEVICE_ARRAY + ['-f', '-y'], check=False)

    def test_create_volume_group(self, run_command):
        run_command.return_value = {
            'returncode': 0
        }
        assert create_volume_group(None, {
            'devices': TESTED_DEVICES,
            'vg_name': TESTED_VG_NAME
        }) == '{}'
        run_command.assert_called_with(['vgcreate', TESTED_VG_NAME] + TESTED_DEVICE_ARRAY, check=False)
    ...
    def test_create_volume_group_with_force(self, run_command):
        ...
        run_command.assert_called_with(['vgcreate', TESTED_VG_NAME] + TESTED_DEVICE_ARRAY + ['-f'], check=False)

    def test_destroy_volume_group(self, run_command):
        run_command.return_value = {
            'returncode': 0
        }
        assert destroy_volume_group(None, {
            'vg_name': TESTED_VG_NAME
        }) == '{}'
    ...
    def test_destroy_volume_group_with_force(self, run_command):
        ...
        run_command.assert_called_with(['vgremove', TESTED_VG_NAME, '-f', '-y'], check=False)
    ...
    def test_create_thin_pool(self, run_command):
        ...
        assert create_thin_pool(None, {
            'vg_name': TESTED_VG_NAME,
            'lv_name': volume_name
        }) == '{}'
        run_command.assert_called_with([
            'lvcreate', '-l', '100%FREE', '-T', '{}/{}'.format(TESTED_VG_NAME, volume_name)
        ], check=False)
