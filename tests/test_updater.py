...
class TestCheckUpdate:
    def test_check_update(self, fs):
        expected = (
            '             [{"name": "Dummy Package", "version": "0.0.0", "release":'
            ' "Dummy released",             "description": "Lorem ipsum...",'
            ' "changelog": null, "url": "http://www.example.com/", "size": "0",        '
...
class TestIsInstalled:
    ...
    def test_one_package_installed(self, run_command):
        run_command.return_value = {"stdout": "test.rpm", "stderr": "", "returncode": 0}

        package = "test"
        result = query_installed(mock.MagicMock(), {"packages": package})
        run_command.assert_called_once_with(["rpm", "-q", package], False)
    ...
    def test_with_many_packages(self, run_command, fs):
        ...
        run_command.assert_called_once_with(["rpm", "-q"] + packages.split(","), False)
...
class TestExceptionLockedFile:
    def test_check_update_with_empty_locked_file(
        self, run_command, timeout, TimeoutException, fs
    ):
        ...
        assert (
            e.value.params[1]
            == "The updater plugin is busy (current operation: <UNKNOWN>)"
        )

    def test_check_update_during_check_update(
        self, run_command, timeout, TimeoutException, fs
    ):
        ...
        assert (
            e.value.params[1]
            == "The updater plugin is busy (current operation: check_update)"
        )

    def test_update_during_check_update(
        self, run_command, timeout, TimeoutException, fs
    ):
        fs.create_file(pytest.plugins_lock_file, contents="check_update")
        run_command.return_value = {}
        timeout.return_value = (
            1  # We don't want to wait a long time in the unit tests. ;)
        )
    ...
    def test_update_during_check_update_before_timeout(
        self, run_command, timeout, TimeoutException, fs
    ):
        fs.create_file(pytest.plugins_lock_file, contents="check_update")
        run_command.return_value = {}
    ...
    def test_ckeck_update_during_update(
        self, run_command, timeout, TimeoutException, fs
    ):
        ...
        assert (
            e.value.params[1]
            == "The updater plugin is busy (current operation: update)"
        )
