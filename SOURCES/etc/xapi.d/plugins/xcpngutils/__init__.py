...
def configure_logging(name):
    ...
    enable_dev_logging_file = (
        "/opt/xensource/packages/files/" + name + "-plugin/devlogging_enabled"
    )
    ...
    def handle_unhandled_exceptions(
        ...
        if not issubclass(exception_type, KeyboardInterrupt):
            log_unhandled_exception(
                "standalone", exception_type, exception_value, exception_traceback
            )
        sys.__excepthook__(exception_type, exception_value, exception_traceback)

    def log_unhandled_exception(
        origin, exception_type, exception_value, exception_traceback
    ):
        logger.error("Nobody caught %s exception: %s" % (origin, exception_type))
        problem = traceback.format_exception(
            exception_type, exception_value, exception_traceback
        )
        for line in problem:
            logger.error(line)
    ...
    def wrapper(*args, **kwds):
        try:
            ...
            raise XenAPIPlugin.Failure(
                str(e.errno), [message, str(e.filename), traceback.format_exc()]
            )
        except ProcessException as e:
            raise_plugin_error(
                e.returncode,
                str(e),
                details="stdout: '%s', stderr: '%s'" % (e.output, e.stderr),
